...
const Create = () => {
     ...
     function handleSubmit(e) {
          e.preventDefault();
          setSending(true);
          Inertia.post(route('purchase.store', [data.store.id, data.purchase.id]), values).then((response) => {
                         ...
                         <form onSubmit={handleSubmit}>
                              <div className="w-full grid grid-cols-3 gap-4">
                                   <div className="p-2 bg-gray-100 text-sm text-gray-600">
                                        <h2 className="text-md text-gray-800 font-semibold border-b pb-1 mb-2">Purchase Order Info</h2>
                                        ...
                                        <tbody>
                                             {values.products.map(({name, qty_order, qty_recieve, cost, tax, total}, i) => {
                                                  return <tr key={`pop${i}`}>
                                                       ...
                                                       <td className="border px-4 py-2 text-center">{qty_order}</td>
...
Create.layout = page => <Layout children={page} header={'Create Purchase'} />;
