...
const Edit = () => {
     ...
     function handleSubmit(e) {
          e.preventDefault();
          setSearching(true);
          Inertia.post(route('purchase-order.update', [data.store.id, data.purchase.id]), values).then((response) => {
               setSearching(false);
          });
     }
          ...
          <React.Fragment key="uprofile">
               ...
               <div className="w-full flex">
                              ...
                              <ul className="overflow-auto h-64 list-none">
                                   {values.results.map((prod, i) => {
                                        return <li key={prod.id} className="w-full p-2 border-b text-md text-gray-600">
                                             <div className="flex">
                                                  <span className="w-5/6">{prod.code} {prod.name}</span>
                    ...
                    <div className="bg-white rounded shadow p-2 w-2/3 ml-2">
                         <h3 className="text-md font-medium text-gray-600 border-b pb-5 item-center">
                         ...
                         <form onSubmit={handleSubmit}>
                              <table className="table col-span-12 mt-2 w-full">
                                   ...
                                   <tbody>
                                        {values.products.map(({id, code, name, qty, remove}, i) => {
                                             return <tr key={i} className={`${remove?'bg-red-300':''}`}>
                                                  <td className={`border px-4 py-2 ${remove?'line-through':''}`}>{code}</td>
                                                  <td className={`border px-4 py-2 ${remove?'line-through':''}`}>{name}</td>
...
Edit.layout = page => <Layout children={page} header={'Edit Purchase Order'} />;
