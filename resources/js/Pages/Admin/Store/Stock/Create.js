...
const Create = () => {
     ...
     function handleSubmit(e) {
          ...
          if(!isError){
               setSending(true);
               Inertia.post(route('stock.store', [data.store.id, data.id]), values).then((response) => {
                    setSending(false);
               });
          }
          return false;
     ...
     return (
          <React.Fragment key="uprofile">
               ...
               <div className="max-w-7xl mx-auto p-2">
                    <InertiaLink className="font-semibold text-md text-gray-700 hover:text-gray-800 leading-tight" href={route('home')}>Dashboard</InertiaLink> | <InertiaLink className="font-semibold text-md text-gray-700 hover:text-gray-800 leading-tight" href={route('store.show', data.store.id)}>{data.store.name}</InertiaLink> | <span className="text-md text-gray-700 leading-tight">Add Stock</span>
               </div>

               <DataContainer>
                    <div className="col-span-12 text-right">
                         <InertiaLink href={route('purchase.index', data.store.id)} className="bg-white border border-gray-400 hover:bg-gray-400 mr-2 text-gray-800 text-sm py-1 px-1 rounded inline-flex items-center">
                              <Icon name={'back'} className={'fill-current w-4 h-4 mr-2'} />
                              back
                         </InertiaLink>
                    </div>

                    <div className="col-span-12 font-semibold text-gray-600">
                         {data.store.name} | {data.code}
                    </div>
                         ...
                         <form onSubmit={handleSubmit}>
                              <div className="flex w-full border-t pt-5">
                                   <table className="table-auto col-span-12">
                                        ...
                                        <tbody>
                                             {values.products.map(({product, quantity, qty_comfirm, expiration, lot, cost, price, duplicate, remove, message}, i) => {
                                                  return <tr key={`pop${i}`} className={message?'bg-red-200':''}>
...
Create.layout = page => <Layout children={page} header={'Add Stock'} />;
