...
const Create = () => {
     ...
     function handleSubmit(e) {
          e.preventDefault();
          setSending(true);
          Inertia.post(route('subcategory.store', data.category.id), values).then(() => {
               setSending(false);
          });
     }
                    ...
                    <ProfileCard>
                         <div className="md:col-span-1">
                              <div className="px-4 sm:px-0">
                                   <h3 className="text-lg font-medium text-gray-900">Create Subcategory</h3>
                                   <p className="mt-1 text-sm text-gray-600">
                                        Create a new subcategory for <span className="font-bold">{data.category.name}</span> category.
                                   </p>
                              </div>
                         </div>
                         <DataCard>
                              <div className="px-4 py-5 sm:p-6">
                                   <div className="grid grid-cols-6 gap-6">
                                        <div className="col-span-12 text-right">
                                             <InertiaLink href={route('subcategory.index', data.category.id)} className="bg-transparent border border-gray-500 text-sm text-gray-500 p-2 rounded focus:outline-none hover:bg-gray-600 hover:text-gray-100 inline-flex items-center">
...
Create.layout = page => <Layout children={page} header={'Create Subcategory'} />;
